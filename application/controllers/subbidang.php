...
class Subbidang extends CI_Controller {
    ...
    public function index()
    {
        ...
        if(!empty($cek)){
            ...
            $d['judul']         ="list_wp";
            $d['judul_halaman'] = "Daftar Sub Bidang";

            $d['all_wp']	    = $this->app_model->get_all_wp();

            $d['content']= $this->load->view('subbidang/view',$d,true);
    ...
    public function tambah(){
        ...
        if(!empty($cek)){
            ...
            $d['judul']='add_wp';
            $d['judul_halaman']='Tambah Subbidang';
            ...
            $text = "SELECT tbl_wbs.wbs_code,tbl_wbs.wbs_name,tbl_wbs.wbs_desc, concat(tbl_wbs.wbs_code,' - ',tbl_wbs.wbs_name) as gabung FROM tbl_wbs";
            $d['l_wbs'] = $this->app_model->manualQuery($text);
			
			$text_lead = "SELECT * FROM tbl_admin";
			$d['l_lead'] = $this->app_model->manualQuery($text_lead);

            $d['content']= $this->load->view('subbidang/form',$d,true);
            $this->load->view('home',$d);
        }else{
    ...
    public function hapus()  {
        ...
        if(!empty($cek)){
            ...
            echo "<meta http-equiv='refresh' content='0; url=".base_url()."index.php/subbidang'>";
        }else{
            header('location:'.base_url());
        }
    }
